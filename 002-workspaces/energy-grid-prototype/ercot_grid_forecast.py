...
def evaluate_forecasts(test_df, sf_forecasts, tgpt_forecasts=None):
    ...
    if tgpt_forecasts is not None:
        ...
        if 'TimeGPT' in tgpt_merged.columns:
            ...
            mape = np.mean(np.abs((tgpt_merged['y'] - tgpt_merged['TimeGPT']) / tgpt_merged['y'])) * 100
            results.append({
                'Model': 'TimeGPT',
                'MAE': mae,
                'RMSE': rmse,
...
def plot_forecasts(train_df, test_df, sf_forecasts, tgpt_forecasts=None,
                   series_id=None, output_path=None):
    ...
    for model, color in colors.items():
        if model in sf_s.columns:
            ax.plot(sf_s['ds'], sf_s[model], '--', color=color,
    ...
    if tgpt_forecasts is not None:
        ...
        if len(tgpt_s) > 0:
            ax.plot(tgpt_s['ds'], tgpt_s['TimeGPT'], 'r-',
                    label='TimeGPT', linewidth=2)
            # Confidence interval
            if 'TimeGPT-lo-90' in tgpt_s.columns:
                ax.fill_between(tgpt_s['ds'],
...
def main():
    ...
    # Plot
    plot_forecasts(
        train_df, test_df, sf_forecasts, tgpt_forecasts,
